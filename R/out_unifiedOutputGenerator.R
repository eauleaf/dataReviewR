#' createTextSummary: create a text based summary of an dataReviewR object
#'
#' @param x an dataReviewR object
...
createTextSummary <- function(x,...) {
  ...
  cat(outputSectionHeader(dataRevHeader))
  cat(newLine)
  ...
  cat(newLine)
  cat(paste0("With dataReviewR version ",x$version, space))
  cat(newLine)
  cat(newLine)
  ...
  if(x$ncolCommon == 0) {
    ...
    #'  cat(newLine)
    #  cat(paste0("Number of rows with some reviewd columns unequal: ", x$nrowSomeUnequal, space))
    #  cat(newLine)
    #  cat(paste0("Number of rows with all reviewd columns equal: " ,x$nrowAllEqual, space))
    #  cat(newLine)
    #}
    cat(newLine)
    ...
    if(x$nrowCommon == 0) {
      cat(paste0("No rows were reviewd, so no summary can be provided"))
    } else {
      cat(paste0("Number of columns reviewd with ALL rows equal: ", x$ncolsAllEqual, space))
      cat(newLine)
      cat(paste0("Number of columns reviewd with SOME rows unequal: ", x$ncolsSomeUnequal, space))
      cat(newLine)
